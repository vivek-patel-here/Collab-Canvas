...
export function MeetingCanvasContextProvider({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        ...
        socketInstance.on("canvas-init", (snapshot: actions[]) => {
            snapshot.map(a => ApplyRemoteAction(a));
        });

        socketInstance.on('canvas-action', (action: any) => {
            ApplyRemoteAction(action);
        });

        return () => {
            socketInstance.off("canvas-init");
            socketInstance.off("canvas-action");
        };
    }, [socketRef.current]);
